...
assert.match(cursor, /-webkit-image-set\(/, '2x glyph so the cursor stays sharp on scaled displays');
...
assert.match(svg, /#fff/, 'white halo');
assert.match(svg, /#(111|1a1d24|000)\b/, 'dark core');
assert.match(svg, /stroke-linecap="butt"/, 'butt caps keep the ring from blobbing into the heads');
assert.match(svg, /A[\d.]+ [\d.]+ 0 /, 'elliptical arc for the circular arrow');
assert.ok((svg.match(/<polygon /g) || []).length >= 2, 'two tangent arrowheads');
...
assert.match(svg, /rotate\(45 16 16\)/);
const nw = context.rotateCursorForHandle({ corner: 'nw' }, 0);
assert.match(decodeURIComponent(nw.match(/^url\("data:image\/svg\+xml,([^"]+)"\)/)[1]), /rotate\(225 16 16\)/);
