...
const child = spawn(electron, ['.', '--remote-debugging-port=0', '--disable-background-timer-throttling', '--disable-renderer-backgrounding', '--disable-features=CalculateNativeWinOcclusion', `--user-data-dir=${profile}`], {
  cwd: root,
  windowsHide: true,
...
const smokeExpression = String.raw`(async()=>{
  ...
  const canvas=document.querySelector('#anViewer'),canvasRect=canvas.getBoundingClientRect(),cw=canvas.width,ch=canvas.height,designWidth=1920,designSize=42*designWidth/1280,projection=cw/designWidth,textContext=canvas.getContext('2d');textContext.save();textContext.font='400 '+designSize+'px "Segoe UI", sans-serif';const rotateMetric=textContext.measureText('Rotate'),rotateProbe=textContext.measureText('Mg');textContext.restore();const rotateAscent=rotateMetric.actualBoundingBoxAscent||rotateProbe.actualBoundingBoxAscent||designSize*.78,rotateDescent=rotateMetric.actualBoundingBoxDescent||rotateProbe.actualBoundingBoxDescent||designSize*.22,halfH=(rotateAscent+rotateDescent)*projection/2,uiScale=cw/canvasRect.width,rotateOffset=30*uiScale,selectionPad=2*uiScale,radius=halfH+selectionPad+rotateOffset;
