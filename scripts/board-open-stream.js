...
async function scanBoardHandle(handle, size) {
  const fileSize = Number(size);
  if (!Number.isSafeInteger(fileSize) || fileSize < 0) throw new Error('Invalid board file size');
  let nextPos = 0;
  let header = Buffer.alloc(0);
  let markerAt = -1;

  while (markerAt < 0 && nextPos < fileSize) {
    const chunk = await readChunk(handle, nextPos);
    if (!chunk.length) break;
    nextPos += chunk.length;
    header = Buffer.concat([header, chunk]);
  ...
  async function appendChunk() {
    if (nextPos >= fileSize) return false;
    const chunk = await readChunk(handle, nextPos);
  ...
  while (true) {
    ...
    while (dataAt < 0) {
      if (scanBuf.indexOf(Buffer.from(']}')) >= 0 || nextPos >= fileSize) {
        return { core, images, size: fileSize };
      }
      if (!await appendChunk()) return { core, images, size: fileSize };
...
async function scanBoardFile(filePath) {
  ...
  try {
    const stat = await handle.stat();
    return await scanBoardHandle(handle, stat.size);
  } finally {
    await handle.close();
  }
}

async function readBoardImageBytesFromHandle(handle, image) {
...
async function readBoardImageBytes(filePath, image) {
  const handle = await fs.open(filePath, 'r');
  try {
    return await readBoardImageBytesFromHandle(handle, image);
  } finally {
    await handle.close();
  }
}
 ...
 * Uses the shared replace helper so a stable .bak survives a crash mid-swap.
 */
