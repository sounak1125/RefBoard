/**
 * Proves the pattern-filled dot grid is pixel-identical to the per-dot loop it
 * replaced, at every device-pixel ratio Windows commonly runs and at pan
 ...
 * loop, copied verbatim, and once with scripts/grid-dots.mjs. Every interior
 * byte must match. Along the clip rectangle's own edge the two cannot: the loop
 ...
 * few units, and the workspace edge is usually off screen anyway. The comparison canvases are accelerated like the board canvas: the
 * software rasteriser shades a half-pixel dot dimmer, so mixing the two would
 * measure the rasteriser, not the grid. Also prints the time both take on a 1440x900 viewport at 2x, for the
...
const smokeExpression = `(async()=>{
  ...
  const make=(W,H,dpr)=>{
    ...
    // No willReadFrequently: that demotes the canvas to the software rasteriser,
    // which anti-aliases the half-pixel dot dimmer than the accelerated one the
    // board actually draws on. Loop and pattern must be compared on the same
    // rasteriser the board uses, and the module's tile is accelerated too.
  ...
  for(const dpr of [1,1.25,1.5,2]){
    for(const [tx,ty] of [[0,0],[3.4,-7.6],[-123.5,456.49],[999.5,-0.5],[4.5,4.5],[-1,-1]]){
      ...
      mod.drawGridDots(gb,{...rect,tx,ty,step,color,dpr});
      const da=ga.getImageData(0,0,a.width,a.height).data;
      const db=gb.getImageData(0,0,b.width,b.height).data;
      let diff=0,maxd=0,dotPixels=0;const where=[];let interiorDiff=0,borderDiff=0,borderMaxChannel=0;
      ...
      for(let i=0;i<da.length;i+=4){
        ...
        if(d){diff++;if(d>maxd)maxd=d;const p=i/4,x=p%a.width,y=Math.floor(p/a.width);
          ...
          else{interiorDiff++;if(where.length<12)where.push({x,y,loop:da[i],pattern:db[i]});}}
        if(da[i]!==bg[0]||da[i+1]!==bg[1]||da[i+2]!==bg[2])dotPixels++;
      }
      cases.push({dpr,tx,ty,pixels:da.length/4,dotPixels,diff,interiorDiff,borderDiff,borderMaxChannel,maxd,where});
    }
  }
  ...
  return {cases,timing:{loopMs,patternColdMs,patternWarmMs}};
})()`;

try {
  const port = await debuggerPort();
  const { cases, timing } = await evaluate(port, smokeExpression, { attempts: 1 });
  const firstBad = cases.find(c => c.interiorDiff > 0 || c.borderMaxChannel > 10);
  ...
  for (const c of cases) {
    ...
    assert.equal(c.interiorDiff, 0, `pattern grid differs from the loop inside the clip at dpr ${c.dpr} offset (${c.tx},${c.ty}): ${c.interiorDiff} px`);
  ...
  console.log(`grid pattern smoke: ${cases.length} cases identical inside the clip, clip-edge delta <= ${border}/255; 1440x900@2x loop ${timing.loopMs.toFixed(2)} ms, pattern cold ${timing.patternColdMs.toFixed(2)} ms, warm ${timing.patternWarmMs.toFixed(2)} ms per frame`);
