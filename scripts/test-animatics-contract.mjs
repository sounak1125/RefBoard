...
assert.match(editor, /rangeControl=e\.target\.matches\('input\[type="range"\]'\)/, 'timeline playback shortcuts must recognize a focused range slider');
assert.match(editor, /e\.code==='Space'&&\(!form\|\|rangeControl\)/, 'Space must keep controlling playback after a volume-slider edit');
...
assert.doesNotMatch(editor, /\.an-drag-ghost[^\n]+transition:/, 'linked drag ghosts must not trail the pointer through independent CSS transitions');
...
assert.match(editor, /\.an-playhead::before[^\n]+left:0/, 'the playhead cap must open into timeline content instead of the fixed gutter');
