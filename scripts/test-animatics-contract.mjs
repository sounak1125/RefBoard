...
assert.match(editor, /\.an-sequence-marker \{[^\n]+left:124px/, 'sequence marker hit targets must begin at the timeline-content boundary');
assert.match(editor, /\.an-sequence-marker::after[^\n]+left:0/, 'sequence marker lines must not extend into the fixed gutter');
assert.match(editor, /\.an-track-add \{ position:sticky; left:132px; z-index:24/, 'add-track controls must remain pinned beside the fixed track-label gutter');
assert.match(editor, /\.an-track-add[^\n]+margin:6px 0 2px 132px/, 'add-track controls must keep their original position to the right of the label gutter');
...
assert.match(editor, /if\(project\.texts\.length\)\{\s*html\+=`<div class="an-track-row"><div class="an-track-label"><b>T1<\/b>/, 'T1 must render above the video tracks only when the project contains text');
